...
### В процессе сделано:
 ...
 - На основе туториала созданы плейбуки Ansible для установки и удалени  KUbernetes в GCP

 - в Makefile добавлены опции для данного ДЗ


Создать отдельную папку the_hard_way в папке kubernetes


• Все созданные в ходе туториала файлы (кроме бинарных)
поместить в папку kubernetes/the_hard_way репозитория
(сертификаты и ключи тоже можно коммитить, но только после
удаления кластера)

Описать установку компонентов kubernetes из туториала
Kubernetes The Hard Way в виде Ansible-плейбуков в папке
kubernetes/ansible

Описать установку компонентов kubernetes из туториала
Kubernetes The Hard Way в виде Ansible-плейбуков в папке
kubernetes/ansible
Задание достаточно выполнить в виде Proof of Concept,
просто автоматизация некоторых действий туториала.

### Как запустить проект:

Для запуска проекта должен быть наcтроен доступ для работы с gcloud  и установлена утилита make

 - Установить необходимые утилиты `kubectl` , `cfssl`, `cfssljson`  командой `make k8s_utils` , которая запускмет плейбук ansible с запросом sudo-пароля
 
 - скопировтаь `inventory.yml.example` в `inventory.yml` и задать свои значения параметров доступа в GCP и рабочую папку, где будут генерироваться сертификаты и другие файлы, в процессе работы плейбуков. Установить свои значения ssh -ключей и пользователя
 
 - запустить установку кластера Kubernetes командой `make k8s_thw_install` , которая запускает соответствующий плейбук

 - Развернуть микросервисы тестового приложения клмандой `make k8s_deploy_reddit` которая заменит парамтеры контейнеров из переменых среды и запустит мангифесты командой  вида `kubectl apply -f ...`

### Как проверить работоспособность:

Запустить `kubectl get pods` , в результате долждны быть в статусе запущен поды `ui`, `post`, `mongo` и `comment`

Для удалени созданных ресурсов, запустить команду `make k8s_clean_thw`  (очистка рабочей папки регулируется переменной `clean_work_path`)
