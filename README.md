...
### В процессе сделано:
 ...
 - Созданы файлы конфигурации  `docker-compose` и `Docker` для развертывания тестового приложения и его мониторинга на базе Prometheus
 - Проверена работа мониторинга 
 - Образы залиты в регистри


Для задания со *  
 - Добавлен экспортер для MongoDB на базе проекта `Percona Mongodb exporter` 
 - Добавьте blackbox мониторинг сервисов `comment`, `post`, `ui` на базе `Cloudprober` 
 - Добавлен `Makefile` для автоматизации создания и заливки образов, а также для запуска и остановки контейнеров


### Как запустить проект:

Предполагается, что настроен доступ к проекту в GCP и установлена утилита make
/Docker/.env.example в /Docker/.env
Задать значение переменной `DOCKERHUB_USERNAME` 
host_ip = docker-machine ip docker-host
C помощью make
make build
make push
make clean
...
### Как проверить работоспособность:
 ...
 - На адресе http://<host_ip>:9292 должен отображаться интерфейс тестового приложения
 - На адресе http://<host_ip>:9090 должен отображаться интерфейс Prometheus
 - На адресе http://<host_ip>:9090/targets должны быть активны  cloudprober comment mongodb node prometheus ui
